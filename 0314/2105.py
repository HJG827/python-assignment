...
def simulate(r, c, len1, len2):
    used = set()
    start_r, start_c = r, c

    cr, cc = r, c
    
    # 첫 번째 변: 방향 0 (아래-오른쪽), L1칸 이동
    # 첫 번째 변: 방향 0 (아래-오른쪽), len1 칸 이동
    for _ in range(1, len1+1):
        cr += dr[0]
        cc += dc[0]
        if not (0 <= cr < N and 0 <= cc < N):
            return -1
        if arr[cr][cc] in used:
            return -1
        used.add(arr[cr][cc])
    
    # 두 번째 변: 방향 1 (아래-왼쪽), len2 칸 이동
    for _ in range(1, len2+1):
        cr += dr[1]
        cc += dc[1]
        if not (0 <= cr < N and 0 <= cc < N):
            return -1
    ...
    # 네 번째 변: 방향 3 (위-오른쪽), len2 칸 중 마지막 한 칸 전까지 이동
    for _ in range(1, len2+1):
        cr += dr[3]
        cc += dc[3]
        if not (0 <= cr < N and 0 <= cc < N):
    ...
    # 마지막 한 칸: 반드시 출발점으로 돌아와야 함
    if (cr, cc) != (start_r, start_c):
        return -1
...
for tc in range(1, T+1):
    ...
    # 전체 시뮬레이션. 사각형의 최대 변 길이 구하기
    for r in range(0, N-2):
        for c in range(1, N-1):
            # 가능한 최대 len1 (첫 번째 변) 계산
            max_len1 = min(N - r - 1, N - c - 1)
            for len1 in range(1, max_len1 + 1):
                # 가능한 최대 len2 (두 번째 변) 계산
                max_len2 = min(N - (r + len1) - 1, c + len1)
                if max_len2 == 0:
                    continue
                for len2 in range(1, max_len2 + 1):
                    result = simulate(r, c, len1, len2)
                    if result != -1:
                        max_desserts = max(max_desserts, result)
    
    print(f'#{tc} {max_desserts if max_desserts != -1 else -1}')