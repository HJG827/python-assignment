...
for tc in range(1, T+1):
    ...
    # gpt가 알려줍니다^^
    # 동적 계획법 이용 (dp[i] = i월까지의 이용금액의 최소합 / dp[12] = 정답!)
    dp = [0]*13
    dp[0] = 0

    for i in range(1, 13):
        # 일별, 월별 이용 금액 비교하고 이전 금액 추가
        dp[i] = dp[i-1] + min(use[i-1]*fee[0], fee[1])

        # 3개월 이상의 경우, 3달 계산 후 3달이용권과 값 비교!
        if i >= 3:
            dp[i] = min(dp[i], dp[i-3]+fee[2])

    # 최종적으로 나온 일, 월, 3개월 비교 최소합과 1년 이용권 비교하여 값 출력
    value = min(dp[12], fee[3])
