...
for _ in range(10):
    ...
    row_sum = max(sum(arr[i])for i in range(100))


    col_sum = max(sum(arr[i][j] for i in range(100))for j in range(100))

    daegag1_sum = sum(arr[i][i] for i in range(100))
    daegag2_sum = sum(arr[i][99-i]for i in range(100))

    result = max(row_sum, col_sum, daegag1_sum, daegag2_sum)
