...
def create_user(lst):
    ...
    for user in lst:
        ...
        if result == 'blocked':
            ...
            error_num += 1
            for kind in result[1]:
                user[kind] = None
            user_list.append(user)
    print(f'잘못된 데이터로 구성된 유저의 수는 {error_num}입니다.')
    print(user_list)
