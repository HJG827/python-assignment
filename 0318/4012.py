...
def unique_splits(N):
    results = []          # 결과를 저장할 리스트 (각 결과는 (group1, group2) 형태로)
    number_lsts = []            # 인덱스 조합을 저장할 리스트

    # 재귀 함수를 사용해 0~n-1 인덱스 중에서 n//2개를 선택하는 조합 생성
    def rec(start, number_lst):
        if len(number_lst) == N // 2:
            number_lsts.append(list(number_lst))
            return
    ...
    # 생성된 인덱스 조합을 이용해 두 그룹을 만듭니다.
    for number_lst in number_lsts:
        # 첫 번째 그룹: comb에 해당하는 인덱스의 숫자들을 정수로 변환하여 리스트로 만듦
        food1 = [int(i) for i in number_lst]
        # 두 번째 그룹: 0~n-1 중 comb에 포함되지 않은 인덱스의 숫자들을 리스트로 만듦
        food2 = [int(i) for i in range(N) if i not in number_lst]
        # 중복 분할을 피하기 위해, 첫 번째 그룹을 하나의 수로 봤을 때(예: 14) 
        # 두 번째 그룹(예: 23)보다 작을 때만 결과에 추가합니다.
        # (비교는 문자열로 연결한 후 정수 변환하여 합니다)
        num1 = int("".join(map(str, food1)))
        num2 = int("".join(map(str, food2)))
        if num1 < num2:
            results.append((food1, food2))
    return results
...
for tc in range(1, T+1):
    ...
    differ = 1000000000000
    splits = unique_splits(N)
